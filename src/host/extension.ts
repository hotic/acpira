...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const runtime = await createHostRuntime(new VscodePlatform(context, log), {
    chatgptBridgePath: vscode.Uri.joinPath(context.extensionUri, 'dist', 'chatgpt-bridge.cjs').fsPath,
  });
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('acpira.connectChatgpt', async () => {
      try {
        const view = await manager.connectChatgpt();
        openEditor(view.id);
        if (view.external?.connectionPrompt) {
          await vscode.env.clipboard.writeText(view.external.connectionPrompt);
          void vscode.window.showInformationMessage(t('chatgpt.copied'));
        }
